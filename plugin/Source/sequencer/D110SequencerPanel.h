...
public:
	...
	// adds to its own layout, exactly as it already does for D110Keyboard::kRefH.
	static constexpr float kRefH = 250.0f;
...
private:
	...
	juce::Rectangle<float> tempoBounds, timeSigBounds, metronomeBounds, precountBounds;
	juce::Rectangle<float> barPrevBounds, barNextBounds, barReadoutBounds;
	juce::Rectangle<float> loadBounds, saveBounds, recModeBounds;
