...
// Old-gen MT-32 hardware only had 5 part-status slots on its LCD. The D-110 (the only
// device this fork's plugin targets) has 8 Parts, so this is raised accordingly. The
// per-part space characters below were also removed to keep the whole line within the
// 20-byte display buffer (8 digits + 'R' + 1 space + up to 8 bytes of volume text = 18).
static const Bit32u DISPLAYED_VOICE_PARTS_COUNT = 8;
...
static void copyNullTerminatedString(Bit8u *destination, const Bit8u *source, Bit32u lengthLimit) {
	...
	// displayBuffer is reused across display modes/updates. Without padding, a short string here
	// leaves stale bytes from whatever longer message previously occupied this region, which then
	// shows up as garbled/overlapping text on screen.
	for (; i < lengthLimit; i++) destination[i] = ' ';
}
	...
	if (lcdUpdateSignalled) {
		...
		case Mode_MAIN: {
			...
			*writePosition++ = ' ';
			...
			Bit8u *endOfContent = writePosition; // Real content ends here - anything past this is stale leftovers.
			...
			// displayBuffer is reused across modes/updates - blank out anything left over from
			// whatever longer message previously occupied the tail of this buffer.
			while (endOfContent < displayBuffer + LCD_TEXT_SIZE) *endOfContent++ = ' ';
			break;
		}
		default:
			break;
