...
Ступень атаки TVA всё равно не едет: правка уровня и времени огибающей регистров не двигает.
Прошивка ставит уровень установкой (без прерывания) и держит его до снятия. Если цепочку
ступеней запускает прерывание, а установка его не даёт, то запускать её должно что-то ещё -
и в захвате этого «чего-то» нет.

Три направления, ни одно пока не проверено:

1. Разобрать длинный путь обработчика с `0x32DC` - там прошивка решает, какую ступень
   грузить дальше, и оттуда видно, чего она для этого ждёт.
2. Проверить, не приходит ли прерывание рампы СРЕЗА раньше, чем нужно, и не сбивает ли оно
   цепочку амплитуды: обе рампы делят один байт состояния и один вывод INT.
3. Взять тембр с заведомо медленной атакой из заводского банка вместо правки нынешнего -
   правка меняет ОЗУ, но, возможно, не то, что прошивка перечитывает при выдаче голоса.
