...
**Следующий опыт, и он различает их прямо**: свести в один поток по времени записи в регистры
рамп и записи в таблицу состояний слотов `edc0` (перехваты для обоих уже есть -
`kLa32TapBase` и `kDispatchTapBase`) и посмотреть, что на железе происходит раньше - запись
регистров или пометка слота занятым. Если пометка раньше, то докладывать сразу правильно, и
`0xFF` - установка. Если позже, то прерывание на настоящей микросхеме приходит уже после
выдачи, и задержка не выдумка, а часть интерфейса.
