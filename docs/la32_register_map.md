...
- **Только синтетический партиал.** Для PCM нужен разбор адреса и длины волны в ПЗУ по
  регистрам `0C40`/`0D00` - его нет.
- **Амплитуда не отмасштабирована**: пик рендера 0.0018 при уровне 126 из 255. У munt `amp` -
  величина в логарифмической области, и как ложится на неё байт уровня, ещё не разобрано.
- **Огибающие в рендер не подаются**: амплитуда и срез держатся постоянными. Сами рампы
  работают (см. выше), но с рендером ещё не сведены.
