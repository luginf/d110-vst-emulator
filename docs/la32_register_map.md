...
- **Срез держится постоянным**: рампа среза (если она есть - см. оговорку выше) в рендер не
  подаётся.
- **Партиалы не смешиваются**: структура тембра решает, складываются они или перемножаются
  кольцевой модуляцией, и `LA32PartialPair` это умеет, но здесь не задействован - PCM и
  синтетический партиал рендерятся и сохраняются порознь.
- **Огибающая подаётся только синтетическому партиалу** в отдельном прогоне; PCM-партиал
  рендерится с постоянным уровнем.
